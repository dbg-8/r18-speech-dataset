...
class TextExecutor(BaseExecutor):
    ...
    def _init_from_path(
        ...
        if cfg_path is None or ckpt_path is None or vocab_file is None:
            ...
            self.cfg_path = os.path.join(
                self.res_path, pretrained_models[tag]['cfg_path']
            )
            self.ckpt_path = os.path.join(
                self.res_path, pretrained_models[tag]['ckpt_path']
            )
            self.vocab_file = os.path.join(
                self.res_path, pretrained_models[tag]['vocab_file']
            )
        else:
    ...
    def _clean_text(self, text):
        ...
        text = re.sub(
            f'[{"".join([p for p in self._punc_list][1:])}]', '', text
        )
        return text
    ...
    def infer(self):
        ...
        """
        if self.task == 'punc':
            input_ids = paddle.to_tensor(self._inputs['input_ids']
                                        ).unsqueeze(0)
            seg_ids = paddle.to_tensor(self._inputs['seg_ids']
                                      ).unsqueeze(0)
    ...
    def execute(self, argv: List[str]) -> bool:
        ...
        try:
            res = self(
                text, task, model_type, lang, cfg_path, ckpt_path,
                punc_vocab, device
            )
            return True
    ...
    def __call__(
        ...
        self._init_from_path(
            task, model, lang, config, ckpt_path, punc_vocab
        )
...
# COPY FROM github.com/PaddlePaddle/PaddleSpeech/paddlespeech/cli/text/infer.py
